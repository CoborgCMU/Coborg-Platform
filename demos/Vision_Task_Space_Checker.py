...
# Usage:
# Update the variables and definitions in the section below the Camera class
# Append as many cameras as desired with their given traits
# Run the program and analyze the graphs produced
...
class Camera:
    ...
    def plot_camera(self):
        ...
        for ii in range(len(camera_r_meshes)):
            ...
            camera_y_meshes.append((camera_r_meshes[ii] * np.sin(camera_theta_meshes[ii]*deg_to_rad)) + self.origin_ft[1])
...
task_space_horizontal_max_deg = 20.0
task_space_horizontal_min_deg = -20.0
task_space_vertical_max_deg = 0
task_space_vertical_min_deg = -60.0
# Hand Spread Definitions
hand_space_rad_diff_ft = 0.0
hand_space_horizontal_angle_diff_deg = 7.0
...
#
# Append cameras
# for camera, y smaller, z smaller is better (far away from chest, and lower place)
cameras.append(Camera(np.array([-4.0/12.0, -14.0/12.0, 7.0/12.0]), R.from_euler('xyz', [0,3,35], degrees=True)))
cameras.append(Camera(np.array([-4.0/12.0, -14.0/12.0, 4.0/12.0]), R.from_euler('xyz', [0,-42, 35], degrees=True)))

# cameras.append(Camera(np.array([4.0/12.0, 0, 1.25]), R.from_euler('xyz', [0,0,0], degrees=True)))
...
for ii in range(len(task_r_meshes)):
    ...
    task_y_meshes.append(task_r_meshes[ii] * np.sin(task_theta_meshes[ii]*deg_to_rad))
...
for ii in range(len(hand_r_meshes)):
    ...
    hand_y_meshes.append(hand_r_meshes[ii] * np.sin(hand_theta_meshes[ii]*deg_to_rad))
...
for ii in range(len(buffer_r_meshes)):
    ...
    buffer_y_meshes.append(buffer_r_meshes[ii] * np.sin(buffer_theta_meshes[ii]*deg_to_rad))
