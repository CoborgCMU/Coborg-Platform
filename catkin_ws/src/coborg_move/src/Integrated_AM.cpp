...
void execute_trajectory_feedback_callback(const moveit_msgs::MoveGroupActionFeedback::ConstPtr& msg)
{
	...
	if (msg->feedback.state == "IDLE" && msg->status.text == "Solution was found and executed.")
	{
		...
		{
			state = 7;
			std::cout<<"Successfully went to ready position."<<std::endl;
		}
		else if (state == 7)
		{
			state = 0;
...
{	
	...
	while (ros::ok())
	{
        ...
        if (state == -1)
        {
			...
			robot_state_ptr->setVariablePositions(plan_req.start_state.joint_state.position);
		...
		else if (state == 6)
		{
			...
			if (plan_res.error_code_.val != plan_res.error_code_.SUCCESS)
			{
				std::cout << "Returning to ready plan failed. planning again." << std::endl;
				continue;
			}
		...
		else if (state == 7)
		{
			...
			if (plan_res.error_code_.val != plan_res.error_code_.SUCCESS)
			{
				std::cout << "Planning to return home failed. planning again." << std::endl;
				continue;
			}
